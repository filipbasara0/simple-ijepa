...
class MaskedImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        context_indices, target_indices_list = self.generate_blocks(num_patches_per_dim)
        return img, context_indices, target_indices_list

    def sample_block(self, num_patches_per_dim, scale_range, aspect_ratio_range):
        ...
        start_row = torch.randint(0, num_patches_per_dim - block_height + 1, (1,)).item()
        start_col = torch.randint(0, num_patches_per_dim - block_width + 1, (1,)).item()

        indices = []
        for i in range(block_height):
            for j in range(block_width):
                indices.append((start_row + i) * num_patches_per_dim + (start_col + j))

        return indices

    def generate_blocks(self, num_patches_per_dim):
        """Generate context and multiple target blocks"""
        context_indices = self.sample_block(num_patches_per_dim, scale_range=(0.85, 1.0), aspect_ratio_range=(0.75, 1.5))
        ...
        for _ in range(self.num_targets):
            ...
            for _ in range(self.num_patches):  # Try up to num_patches iterations
                target_indices = self.sample_block(num_patches_per_dim, scale_range=(0.15, 0.2), aspect_ratio_range=(0.75, 1.5))
                overlap = len(set(context_indices) & set(target_indices)) + len(all_target_indices & set(target_indices))
                if overlap == 0:
        ...
        context_indices = [idx for idx in context_indices if not any(idx in target_indices for target_indices in target_indices_list)]
...
def collate_fn(batch):
    ...
    min_target_size = min(min(len(indices) for indices in target_list) for target_list in target_indices_list_list)

    # Truncate context and target indices to the minimum size
    context_indices_list = [indices[:min_context_size] for indices in context_indices_list]
    target_indices_list_list = [[indices[:min_target_size] for indices in target_list] for target_list in target_indices_list_list]
